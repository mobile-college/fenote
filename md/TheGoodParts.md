...
String.method ('trim', function {
  return this.replace(/藛\s+|\s+$/g, '');     //uses regular expression
});
```
...
* `(?:\/([藛?#]*))?`
  ...
  * The `藛` at the beginning of character class `[藛?#]` means it includes _all_ characters _except_ ? and #
...
* Note: **`藛` and `$` are important because they anchor the regexp** and checks whether the string matched against it contains _only_ what is in the regexp
  * If `藛` and `$` weren't present, it would check that the string _contained_ the regexp but wouldn't necessarily be only made up of this
  * Using only `藛` checks the string _starts_ with the regexp
...
3 flags exist in regular expressions: `i` means insensitive - ignore the character case, 'g` means global - to match multiple items and `m` means multiline - where 藛 and $ can match line-ending characters
...
* The following special characters must all be _escaped_ with a backslash `\` to be taken literally, or they will take on an alternative meaning: \ / [ ] ( ) { } ? + * | . 藛$
* The `\` prefix **does not make letters or digits literal**
* When unescaped:
  * `.` matches any character except line-ending
  * `藛` matches the beginning of the text when `lastIndex` property is zero, or matches line-ending character when the `m` flag is present
  * Having `藛` inside a [character class](#RegexpClass) means NOT, so [藛0-9] means _does not_ match a digit
...
* `\d` === [0-9] and `\D` is the opposite, NOT (藛) a digit, [藛0-9]
* `\s` matches is a partial set of Unicode whitespace characters and `\S` is the opposite
* `\w` === [0-9A-Za-z] and `\W` === [藛0-9A-Za-z] but **useless for any real world language** (because of accents on letters, etc)
...
* Can shorten specification of all 32 ASCII special characters to **[!-\/:-@\[-'{-藴]** (note that the ' in this piece of code should be a back-tick which I can't use as part of these notes)
* Also allows `藛` as the first character after the opening `[` to mean _NOT_ the characters in the character set

####Regexp Class Escape

There are **_specific_ characters that must be escaped in a character class**: -  /  [  \  ]  藛
...
If the **_regexp_ contains a `g` flag** (e.g. `var regexp = /[藛<>]+|<(\/?)([A-Za-z]+)([藛<>]*)>/g;`), there is a lot more to look out for:
...
* If searching for multiple occurrences of a pattern by calling `exec` in a loop, ensure you _reset `lastIndex`_ when exiting the loop and remember `藛` only matches _when `lastIndex` is equal to zero_
...
* **Bitwise operators**: Shouldn't really be doing this kind of manipulations because they are quite slow in JavaScript, therefore there shouldn't be a need to use `&`, `|`, `藛`, `藴`, `>>`, `>>>` or `<<`
